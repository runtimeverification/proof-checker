...
@pytest.mark.parametrize('pattern, evar_id, plug, expected', [
    ...
    [SSubst(MetaVar(0), SVar(0), Symbol(1)), 0, Symbol(1), ESubst(SSubst(MetaVar(0), SVar(0), Symbol(1)), EVar(0), Symbol(1))]
])

def test_apply_esubst(pattern: Pattern, evar_id: int, plug: Pattern, expected: Pattern):
...
def test_instantiate_esubst(stack: Callable[[MetaVar], Pattern], mvar, plugs, expected):
    assert stack(mvar).instantiate(plugs) == expected


@pytest.mark.parametrize('pattern, svar_id, plug, expected', [
    # Atomic cases
    ...
    # Subst when evar_id is fresh should do nothing
    [MetaVar(0, s_fresh=[0, 1]), 0, Symbol(1), MetaVar(0, s_fresh=[0, 1])],
    # Subst on substs should stack
    [ESubst(MetaVar(0), EVar(0), Symbol(1)), 0, Symbol(1), SSubst(ESubst(MetaVar(0), EVar(0), Symbol(1)), SVar(0), Symbol(1))],
    [SSubst(MetaVar(0), SVar(0), Symbol(1)), 0, Symbol(1), SSubst(SSubst(MetaVar(0), SVar(0), Symbol(1)), SVar(0), Symbol(1))]
])
def test_apply_ssubst(pattern: Pattern, svar_id: int, plug: Pattern, expected: Pattern):
    assert pattern.apply_ssubst(svar_id, plug) == expected


# Subst 0 for 1 and then 1 for 2
esubst_stack_seq = lambda term: SSubst(SSubst(term, SVar(0), plug=SVar(1)), SVar(1), plug=SVar(2))

# Subst 1 for 2 and then 0 for 1
esubst_stack_seq_rev = lambda term: SSubst(SSubst(pattern=term, var=SVar(1), plug=SVar(2)), var=SVar(0), plug=SVar(1))

@pytest.mark.parametrize('stack, mvar, plugs, expected', [
    [esubst_stack_seq, MetaVar(0), {0: SVar(0)}, SVar(2)],
    [esubst_stack_seq, MetaVar(0), {0: SVar(1)}, SVar(2)],
    [esubst_stack_seq, MetaVar(1), {0: SVar(1)}, esubst_stack_seq(MetaVar(1))],
    [esubst_stack_seq, MetaVar(0), {0: MetaVar(1)}, esubst_stack_seq(MetaVar(1))],
    [esubst_stack_seq_rev, MetaVar(0), {0: SVar(0)}, SVar(1)],
    [esubst_stack_seq_rev, MetaVar(0), {0: MetaVar(1)}, esubst_stack_seq_rev(MetaVar(1))],
    [stack_mixed1, MetaVar(0), {0: SVar(1)}, EVar(2)],
    [stack_mixed1, MetaVar(0), {0: SVar(2)}, SVar(2)]
])

def test_instantiate_ssubst(stack: Callable[[MetaVar], Pattern], mvar, plugs, expected):
    assert stack(mvar).instantiate(plugs) == expected