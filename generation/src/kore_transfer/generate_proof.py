...
def generate_proofs(
    hints: Iterator[KoreHint], proof_expression: type[KoreDefinition], language_definition: LanguageSemantics
) -> None:
    claims = 0
    for hint in hints:
        axiom = proof_expression.add_axioms(hint, language_definition)
